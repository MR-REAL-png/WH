  ...
  list.innerHTML = items
    ...
    <div class="item-card">
      <div class="item-card__top">
        ...
        <span class="badge badge--highrack">Highrack</span>
      </div>
...
async function removeFromRak(sku) {
  ...
  showToast('Barang dikeluarkan dari rak, status kembali ke Unpack', 'default');
  await renderRakItems();
}
  ...
  container.innerHTML = list
    ...
    <div class="item-card" data-sku="${escapeHtml(b.sku)}" style="padding:12px 14px;">
      <div class="item-card__top">
        ...
        ${b.lokasi_rak ? `<span class="badge badge--highrack">${escapeHtml(b.lokasi_rak)}</span>` : ''}
      </div>
    </div>`
    )
    .join('');
